...
$a['ventes'] = null; $a['stock'] = null; $a['retours'] = null; $a['panier'] = null; $a['perdus'] = null;
...
$content .= '
    <form method="get">
        ...
        <p>
            ...
            <input type="text" name="stock_stockage" id="stock_stockage" value="'.(isset($_GET['stock_stockage']) ? $_GET['stock_stockage'] : null).'" />
        </p>

        <p>
            <label for="publisher_id">Éditeur n&deg; :</label>
            <input type="number" class="short" name="publisher_id" id="publisher_id" value="'.(isset($_GET['publisher_id']) ? $_GET['publisher_id'] : null).'" />
        </p>
        ...
        <p>
            ...
            <input type="number" class="mini" name="stock_limit" id="stock_limit" value="'.(isset($_GET['stock_limit']) ? $_GET['stock_limit'] : null).'" /> exemplaires
        </p>
...
if (isset($_GET['stock_limit']) && !empty($_GET['stock_limit'])) {
...
foreach ($sql as $x) {
    ...
    if (!empty($x['stock_id'])) {
        ...
        if ($x['stock_return_date']) { // Retourné
            ...
            // Sold in shop
            if ($x['customer_id'] === $globalSite->getOpt('fake_shop_customer')) {
        ...
        $line .= '
                ...
                <td class="center">
                    <img src="/common/img/'.$x['led'].'.png" width="8" height="8" />&nbsp;'.$x['status'].'
                </td>
            </tr>
        ';
        ...
        $list .= $line;
        if ($x['stock_return_date']) {
            $tr_retours .= $line;
        } elseif ($x['stock_selling_date']) {
            $tr_ventes .= $line;
        } elseif ($x['stock_cart_date']) {
            $tr_paniers .= $line;
        } else {
            $tr_stock .= $line;
        }
...
$lists = $lm->getAll(['site_id' => LegacyCodeHelper::getGlobalSite()['site_id']]);
