    ...
    foreach ($copies as $copy) {
        ...
        if ($order->isPayed() && $article->getType()->isDownloadable()) {
            if ($article->isPublished()) {
                ...
                if ($files) {
                    foreach ($files as $f) {
                        $a['dl_links'] .= ' <a href="' . $f->getUrl(LegacyCodeHelper::getGlobalVisitor()['user_key']) . '" title="' . $f->get('version') . ' | ' . file_size($f->get('size')) . ' | ' . $f->getType('name') . '"><img src="' . $f->getType('icon') . '" width=16 alt="Télécharger"> ' . $f->get('title') . '</a> &nbsp;';
                    }
...
function _orderBelongsToVisitor(Order $order, CurrentUser $currentUser): bool
{
    ...
    return $order->get("axys_account_id") === $currentUser->getAxysAccount()->getId();
}
