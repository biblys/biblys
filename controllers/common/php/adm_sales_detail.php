...
$dates = null;
$days = $_SQL->query("SELECT DATE_FORMAT(`order_payment_date`, '%Y-%m-%d') as `date`,`order_payment_date` FROM `orders` WHERE `orders`.`site_id` = '". LegacyCodeHelper::getGlobalSite()["site_id"]."' AND `order_payment_date` > SUBDATE(NOW(), INTERVAL 1 MONTH) AND `order_cancel_date` IS null GROUP BY `date` ORDER BY `date` DESC");
while ($s = $days->fetch()) {
...
// Raccourci mois
$months = null;
$mois = $_SQL->query("SELECT DATE_FORMAT(`order_payment_date`, '%Y-%m') as `date`,`order_payment_date` FROM `orders` WHERE `orders`.`site_id` = '". LegacyCodeHelper::getGlobalSite()["site_id"]."' AND `order_cancel_date` IS null GROUP BY `date` ORDER BY `date` DESC");
while ($m = $mois->fetch()) {
