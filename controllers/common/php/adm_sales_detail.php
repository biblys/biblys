...
$dates = null;
$days = $_SQL->query("SELECT DATE_FORMAT(`order_payment_date`, '%Y-%m-%d') as `date`,`order_payment_date` FROM `orders` WHERE `order_payment_date` > SUBDATE(NOW(), INTERVAL 1 MONTH) AND `order_cancel_date` IS null GROUP BY `date` ORDER BY `date` DESC");
while ($s = $days->fetch()) {
...
// Raccourci mois
$months = null;
$mois = $_SQL->query("SELECT DATE_FORMAT(`order_payment_date`, '%Y-%m') as `date`,`order_payment_date` FROM `orders` WHERE `order_cancel_date` IS null GROUP BY `date` ORDER BY `date` DESC");
while ($m = $mois->fetch()) {
...
$stock = $_SQL->prepare('SELECT
    ...
    WHERE `order_payment_date` IS NOT null'.$_QUERY.'
    GROUP BY `s`.`stock_id`
